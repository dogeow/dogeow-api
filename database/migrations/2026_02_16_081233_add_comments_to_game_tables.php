    ...
    public function up(): void
    {
        ...
        foreach ($columnComments as $table => $columns) {
            foreach ($columns as $column => $comment) {
                // 获取字段类型并添加注释
                $columnType = DB::selectOne("SELECT COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?", [$table, $column]);
                if ($columnType) {
                    $type = $columnType->COLUMN_TYPE;
                    $isNullable = DB::selectOne("SELECT IS_NULLABLE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?", [$table, $column])->IS_NULLABLE;
                    $nullClause = $isNullable === 'YES' ? 'NULL' : 'NOT NULL';
                    DB::statement("ALTER TABLE `{$table}` MODIFY `{$column}` {$type} {$nullClause} COMMENT '{$comment}'");
                }
            }
        }
    }
